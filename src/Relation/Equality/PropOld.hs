module Relation.Equality.PropOld where
...
-- TODO: cool way of doing equality chains using EqualProp...
-- its difficult because the first term has to be a raw term, but then chained
-- terms  will contain proof witnesses. and i cant just ignore witnesses and
-- pass around eqprop
...
-- -- TODO: try converting constructors to axioms that just work on the predicate logic of eqprop

-- {-@
-- (=~) ::
--   x:a ->
--   {yw:(a, EqualityProp a) | eqprop x (fst yw) (snd yw)} ->
--   EqualProp a {x} {fst yw}
-- @-}
...
-- {-@
-- (=~=) ::
--   {xw:(a, EqualityProp a) | eqprop ??? (fst xw) (snd xw)} ->
--   {yw:(a, EqualityProp a) | eqprop (fst xw) (fst yw) (snd yw)} ->
--   EqualProp a {???} (fst yw)
-- @-}
