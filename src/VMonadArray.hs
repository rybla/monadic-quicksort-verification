...
vreadList iMonadArray i (Suc n) = vliftF2_ Cons
                                           (vread_ i)
                                           (vreadList_ (Suc i) n)
 where
...
vwriteList iMonadArray i Nil = vlift_ ()
 where
  vlift_  = vlift iMonad_
  iMonad_ = iMonad iMonadArray
vwriteList iMonadArray i (Cons x xs) = vseq_
  (vwrite_ i x)
...
vwriteListToLength iMonadArray i xs = vseq_ (vwriteList_ i xs)
                                            (vlift_ (vlength xs))
 where
  vwriteList_ = vwriteList iMonadArray
  vseq_       = vseq iMonad_
...
vwriteList2ToLength
  ...
  -> VTuple2D (VList a)
  -> m (VTuple2D VNat)
vwriteList2ToLength iMonadArray i (xs, ys) = vseq_
...
vwriteList3ToLength
  ...
  -> VTuple3D (VList a)
  -> m (VTuple3D VNat)
vwriteList3ToLength iMonadArray i (xs, ys, zs) = vseq_
